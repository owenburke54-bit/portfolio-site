...
import {
  ...
  CardHeader,
  CardTitle,
} from "@/components/fin-advisor/ui/Card";
...
export default function ProfileCard() {
  ...
  function handleSave() {
    const parsed = profileSchema.safeParse({
      ...
      monthlyContribution:
        form.monthlyContribution !== undefined ? Number(form.monthlyContribution) : undefined,
    });
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <label className="mb-1 block text-sm">Risk level: {form.riskLevel} / 5</label>
            <Slider
              min={1}
              max={5}
              step={1}
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  riskLevel: Number(e.target.value) as UserProfile["riskLevel"],
                })
              }
            />
          </div>
          ...
          <div>
            <label className="mb-1 block text-sm">Investment horizon (years)</label>
            <Input
              type="number"
              value={form.investmentHorizonYears}
              onChange={(e) =>
                setForm({ ...form, investmentHorizonYears: Number(e.target.value) })
              }
            />
          </div>

          <div>
            <label className="mb-1 block text-sm">Primary goal</label>
            <Select
          ...
          <div className="sm:col-span-2">
            <label className="mb-1 block text-sm">Monthly contribution (optional)</label>
            <Input
              type="number"
          ...
          <div className="sm:col-span-2">
            <label className="mb-1 block text-sm">Goal description (optional)</label>
            <Input
